...
app.post('/run', validateBridgeSignature, enforceImmutableIds, async (req, res) => {
  ...
  try {
    ...
    replayHooks.hookExecutionResponse(trace_id, execution_id, 'completed', { artifact_location: bucketResponse.data.location, artifact_hash: bucketResponse.data.hash, duration_ms: duration });
    ...
    res.json({
      ...
      artifact_hash: bucketResponse.data.hash,
      duration_ms: duration
    });
  } catch (err) {
